...
from .networks import fcn8_vgg16, unet_resnet
...
class SemSeg(torch.nn.Module):
    ...
    def train_on_batch(self, batch):
        ...
        elif loss_name in 'tversky_loss':
            logits = self.model_base(images)
            # full supervision
            loss = losses.compute_tversky_loss(images, logits, masks=batch["masks"].cuda())

        elif loss_name in 'dice_loss':
            logits = self.model_base(images)
            # full supervision
            loss = losses.compute_dice_loss(images, logits, masks=batch["masks"].cuda())
        ...
        elif loss_name in 'focal_cross_entropy':
            logits = self.model_base(images)
            # full supervision
            loss = losses.compute_focal_cross_entropy(images, logits, masks=batch["masks"].cuda())
