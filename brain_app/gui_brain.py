...
from utils import *
SIZE_OF_CANVAS_WiDTH = 200
SIZE_OF_CANVAS_HEIGH = 180


root = tk.Tk()
message = tk.messagebox.showinfo("Notification","you need to put T1w and T2w in the same root folder!")
root.title("BRAIN SEGMENTATION BY MINH NHAT TRINH")
root.iconbitmap("brain.ico")
img = Image.open("hust.png")
img = img.resize((600,90))
photo = ImageTk.PhotoImage(img)
imgDeco = tk.Label(root, image = photo ).grid(row=0, column=0, columnspan=4, rowspan=3)
model = load_model("model_dense.h5", 
                    custom_objects={"Swish":Swish}, compile=False)
